...
import { useSubmitItServices } from "@/hooks/use-service-requests";
...
const IT_SERVICE_OPTIONS = IT_SERVICE_TYPE_OPTIONS;

const STEPS: HowItWorksStep[] = [
  {
    icon: ClipboardList,
    title: "Pick a service",
    description: "Web design, server tech, networking, or other IT help.",
  },
  {
    icon: Users,
...
export default function ItServicesPage() {
  ...
  const mutation = useSubmitItServices();
  const router = useRouter();
  const { data: categories = [], isLoading: categoriesLoading } = useCategories();

  const form = useForm<ItServicesFormValues>({
    resolver: zodResolver(itServicesSchema),
    defaultValues: {
      name: "",
      phone: "",
      email: "",
  ...
  const onSubmit = async (values: ItServicesFormValues) => {
  ...
  return (
    <main className="pb-20">
      ...
      <HowItWorks
        heading="How our IT services work"
